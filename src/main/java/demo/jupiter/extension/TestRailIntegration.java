...
public class TestRailIntegration {
    ...
    public TestRail setUpTestrailInstance() {
        ...
        return TestRail.builder(cfg.testRailUrl( ), cfg.testRailUserName( ), cfg.testRailPassKey( ))
                .applicationName("Hal Projects")
                .build( );
    }
    ...
    public int returnProject(TestRail testRail) throws TestRailException {
        ...
        return projects.stream( )
                .filter(project -> project.getName( ).equalsIgnoreCase(cfg.testRailProject( )))
                .findFirst( )
                .map(Project::getId)
                .get( );
    }
    ...
    public int returnRun(TestRail testRail, int projectId) throws TestRailException {
        ...
        return runs.stream( )
                .filter(run -> run.getName( ).equalsIgnoreCase(cfg.testRailRun( )))
                .findFirst( )
                .map(Run::getId)
                .get( );
    }
    ...
    public boolean checkTestCaseTestRun(int testCaseid, int runId, TestRail testRail) {
        ...
        ArrayList<Integer> tcid = testCases.stream( )
                .map(Test::getCaseId)
    ...
    public void updateTestCaseStatus(TestRail testRail, int runId, int testcaseId, int statusCode) throws TestRailException {
        ...
        if ( checkTestCaseTestRun(testcaseId, runId, testRail) )
            testRail.results( )
                    .addForCase(runId, testcaseId, new Result( ).setStatusId(statusCode), testRail.resultFields( )
                            .list( )
                            .execute( ))
                    .execute( ).getStatusId( );
